...
def so2SO(so_data):
    ...
    return eulerAngles2rotationMat(so_data, format='rad')
...
def ses2poses_quat(data):
    ...
    # data_size = data.shape[0]
    # all_pose_quat = np.zeros((data_size + 1, 7))  # 放四元数的容器
    # all_pose_quat[0, :] = np.array([0., 0., 0., 0., 0., 0., 1.])
    # pose = np.array([0., 0., 0., 0., 0., 0.])
    # for i in range(0, data_size):
    #     pose=pose+data[i,:]
    #     quat = R.from_euler('xyz',[pose[3],pose[4],pose[5]]).as_quat()
    #     # data_mat = se2SE(data[i, :])  # data_mat为两张图片间相对位姿对应的变换矩阵
    #     # pose = pose * data_mat  # 以设定的单位pose为基准，计算各图像帧的绝对位姿
    #     # quat = SO2quat(pose[0:3, 0:3])
    #     all_pose_quat[i + 1, :3] = np.array([pose[0], pose[1], pose[2]])
    #     all_pose_quat[i + 1, 3:] = np.array([quat[0], quat[1], quat[2],quat[3]])
    # return all_pose_quat  # 返回绝对位姿的四元数，此时第一帧好像没放里边
