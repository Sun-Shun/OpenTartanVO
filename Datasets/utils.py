...
class Compose(object):
    ...
    Example::
        ...
        >>> Compose([CropCenter((448, 640)), DownscaleFlow(), ToTensor()])
    """
...
class DownscaleFlow(object):
    """Scale the flow / intrinsics / mask to a fixed size."""
...
class CropCenter(object):
    """Crops a sample of data at centre.
    If the image is smaller than the target size, it is first resized
...
class ToTensor(object):
    """Convert ndarrays in sample to Tensors and normalise RGB images to [0, 1]."""
    ...
    def __call__(self, sample):
        ...
        for kk in kks:
            ...
            if len(data.shape) == 3:   # (H, W, C) -> (C, H, W)
                data = data.transpose(2, 0, 1)
            elif len(data.shape) == 2:
                data = data.reshape((1,) + data.shape)

            if len(data.shape) == 3 and data.shape[0] == 3:  # normalise RGB
                data = data / 255.0
...
def tensor2img(tensImg, mean, std):
    """Convert a normalised tensor back to a uint8 numpy image."""
...
def visflow(flownp, maxF=500.0, n=8, mask=None, hueMax=179, angShift=0.0):
    """Visualise an optical flow field in KITTI colour coding."""
...
def dataset_intrinsics(dataset='tartanair'):
    """Return (focalx, focaly, centerx, centery) for a known dataset."""
    if dataset == 'kitti':
...
def plot_traj(gtposes, estposes, vis=False, savefigname=None, title=''):
    """Plot ground-truth and estimated trajectory (top-down view)."""
...
def make_intrinsics_layer(w, h, fx, fy, ox, oy):
    """Build a 2-channel intrinsics layer (normalised pixel coordinates)."""
...
def load_kitti_intrinsics(filename):
    ...
    focalx = float(cam_intrinsics[0])
    focaly = float(cam_intrinsics[5])
    centerx = float(cam_intrinsics[2])
    centery = float(cam_intrinsics[6])
    return focalx, focaly, centerx, centery
