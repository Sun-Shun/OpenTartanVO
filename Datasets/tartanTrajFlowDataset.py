...
class TrajFolderDataset(Dataset):
    """Dataset that loads consecutive image pairs from a folder for VO evaluation."""
    ...
    def __init__(self, imgfolder, posefile=None, transform=None,
        ...
        self.rgbfiles = [
            (imgfolder + '/' + ff)
            for ff in files if (ff.endswith('.png') or ff.endswith('.jpg'))
        ]
        self.rgbfiles.sort()
        ...
        if posefile is not None and posefile != "":
            ...
            assert poselist.shape[1] == 7, "Pose file must have 7 columns [x y z qx qy qz qw]"
            ...
            assert len(self.motions) == len(self.rgbfiles) - 1, \
                "Number of motions must equal number of images minus 1"
        else:
            self.motions = None
    ...
    def __getitem__(self, idx):
        ...
        h, w, _ = img1.shape
        intrinsicLayer = make_intrinsics_layer(w, h, self.focalx, self.focaly,
