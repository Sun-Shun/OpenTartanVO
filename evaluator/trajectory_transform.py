...
def shift0(traj):
    ...
    '''
    traj_ses = pos_quats2SE_matrices(np.array(traj))  # 将四元数变为变换矩阵T
    traj_init = traj_ses[0]  # 初始相对位姿对应的变换矩阵T
...
def rescale(poses_gt, poses):
    ...
    speed_square_gt = np.sum(trans_gt * trans_gt, 1)  # 先将trans_gt矩阵按元素相乘，然后将结果按每一行相加
    speed_gt = np.sqrt(speed_square_gt)
    speed_square = np.sum(trans * trans, 1)
    speed = np.sqrt(speed_square)  # speed = [xx,xx,xx,xx]
    # when the speed is small, the scale could become very large
    # import ipdb;ipdb.set_trace()
    mask = (speed_gt > 0.0001)  # * (speed>0.00001)  # mask= [False，True]
    scale = np.mean((speed[mask]) / speed_gt[mask])  # (speed[mask]) / speed_gt[mask] 只返回含为True的数组放入数组[x,x,x]
    scale = 1.0 / scale
