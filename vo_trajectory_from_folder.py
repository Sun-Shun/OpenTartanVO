...
def get_args():
    parser = argparse.ArgumentParser(description='TartanVO Monocular Visual Odometry')
    ...
    parser.add_argument('--test-dir', default='',
                        help='test trajectory folder containing RGB images (default: "")')
    parser.add_argument('--pose-file', default='',
                        help='ground-truth pose file for scale calculation and '
                             'visualisation (default: "")')
    parser.add_argument('--save-flow', action='store_true', default=False,
                        help='save optical flow images (default: False)')
...
if __name__ == '__main__':
    ...
    testDataset = TrajFolderDataset(
        args.test_dir, posefile=args.pose_file, transform=transform,
    ...
    if args.pose_file.endswith('.txt'):
        ...
        results = evaluator.evaluate_one_trajectory(
            args.pose_file, poselist, scale=True,
        ...
        if datastr == 'euroc':
            ...
            print("==> ATE: %.4f,\t KITTI-R/t: %.4f, %.4f" % (
                results['ate_score'],
                results['kitti_score'][0],
                results['kitti_score'][1]))

        plot_traj(results['gt_aligned'], results['est_aligned'],
                  vis=False,
                  savefigname='results/' + testname + '.png',
